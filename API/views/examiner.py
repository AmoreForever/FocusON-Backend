        ...
        try:
            ...
            if serializer.is_valid():
                ...
                data = {
                    ...
                    "exam_startdate": request.POST.get("exam_startdate"),
                    "exam_duration": request.POST.get("exam_duration"),
                }

                return JsonResponse(
                    {"message": "Exam created", "data": request.data},
                    status=status.HTTP_201_CREATED,
                )
            return JsonResponse(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except Examiner.DoesNotExist:
            return JsonResponse(
                {"message": "Examiner not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def post(self, request, id):
        ...
        try:
            ...
            for username in usernames:
                ...
                data["id"] = user.id
                supervisors_ids = (
                    AllowedStudents.objects.filter(exam=exam)
                    .values("supervisor")
                    .annotate(total=Count("supervisor"))
                    .order_by("total")
                )
                ...
                if serializer.is_valid():
                    ...
                    reassign_supervisors = reassign_supervisors_to_students(
                        supervisors, exam
                    )
                else:
                    return Response(
    ...
    def get(self, request, id):
        try:
            ...
            for supervisor in exams_supervisors:
                ...
                students = AllowedStudents.objects.filter(
                    supervisor=supervisor.supervisor, exam=exam
                )
                supervisor_students = []
...
class OneSupervisorView(APIView):
    ...
    def delete(self, request, id, sp):
        try:
            ...
            for supervisor in supervisors:
                assigned_supervisors_names.append(supervisor.supervisor.user.username)
            assign_supervisors = reassign_supervisors_to_students(
                assigned_supervisors_names, exam
            )
            return Response(status=status.HTTP_200_OK)
        except:
            return Response(status=status.HTTP_404_NOT_FOUND)
...
class ExamStatisticsView(APIView):
    ...
    def get_questions_statistics(self, exam):
        try:
            ...
            for question in questions:
                ...
                all_students_answers = StudentAnswer.objects.filter(
    ...
    def get(self, request, id):
        try:
            ...
            data = {}
            data["id"] = exam.id
            data["total_mark"] = Question.objects.filter(exam=exam).aggregate(
                total_mark=Sum("mark")
            )["total_mark"]
            data["name"] = exam.exam_name
            data["exam_statistics"] = exam_results.aggregate(
                avg=Avg("mark"),
                max=Max("mark"),
                min=Min("mark"),
                num_of_students_submited_the_exam=Count("student"),
                standard_deviation=StdDev("mark"),
            )
...
class DuringExamView(APIView):
    ...
    def get(
        self, request, id
    ):  # This method can be used for examiner, student and supervisor.
        try:
            exam = Exam.objects.get(id=id)
...
class MostRecentExamView(APIView):
    ...
    def get(self, request):
        try:
            ...
            for exam in exams:
                mins, hrs = math.modf(exam.exam_duration)
                exam_endtime = exam.exam_startdate + timedelta(
                    hours=hrs, minutes=mins * 60
                )
                if not timezone.now() < exam_endtime:
                    data["id"] = exam.id
                    return Response(data=data, status=status.HTTP_200_OK)
    ...
    def post(self, request, id):
        ...
        try:
            ...
            for username in usernames:
                user = User.objects.get(username=username)
                student = Student.objects.get(user=user)
                allowed_students = ProgrammingTestAllowedStudents.objects.filter(
...
class StudentProgrammingAnswerView(APIView):
    ...
    def get(self, request, id, st):
        try:
            ...
            if test.examiner.pk != request.user.id:
                return Response(status=status.HTTP_401_UNAUTHORIZED)
            student_answers = StudentProgrammingAnswer.objects.filter(
                test=test, student=student
            )
