...
class StudentAllExamsSerializer(serializers.ModelSerializer):
    ...
    def get_exam_full_mark(self, obj):
        student_mark = self.get_student_mark(obj)
        if student_mark is None:
            student_mark = 0  # or any default value you want
        exam_full_mark = 100  # or however you calculate it
        exam_full_mark += student_mark
        return exam_full_mark

    def get_student_mark(self, obj):
        try:
            return ExamResults.objects.get(
                exam=obj.exam.id, student=obj.student.user.id
            ).mark
